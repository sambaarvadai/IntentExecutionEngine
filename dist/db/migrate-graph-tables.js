...
exports.migrateGraphTablesToSeparateDB = migrateGraphTablesToSeparateDB;
...
async function migrateGraphTablesToSeparateDB() {
    console.log('🔄 Starting graph table migration...');
    const mainDb = await (0, sqlite_1.getDatabase)();
    const graphDb = await (0, db_1.getGraphDatabase)();
    try {
    ...
    `);
        ...
        console.log(`📋 Found ${tables.length} graph-related tables in main database:`, tables.map(t => t.name));
        ...
        `, tableName);
                ...
                if (count.count > 0) {
                    ...
                    try {
                        ...
                        for (const row of rows) {
                            ...
                            await graphDb.run(`INSERT OR REPLACE INTO ${tableName} (${columns.join(', ')}) VALUES (${placeholders})`, ...values);
                        }
                ...
                console.log(`⚠️  Could not process table ${tableName}:`, error.message);
            }
        }
...
//# sourceMappingURL=migrate-graph-tables.js.map