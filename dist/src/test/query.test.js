...
describe('Query Node Factories', () => {
    ...
    describe('buildFilteredQueryNode', () => {
        it('creates a filtered query node', () => {
            const node = (0, query_1.buildFilteredQueryNode)({
    ...
    describe('buildPaginatedQueryNode', () => {
        it('creates a paginated query node', () => {
            const node = (0, query_1.buildPaginatedQueryNode)({
            ...
            expect(node.plan?.limit).toBe(10);
            expect(node.plan?.offset).toBe(20);
            expect(node.plan?.orderBy).toEqual({ field: 'name', direction: 'asc' });
        });
        it('handles optional parameters', () => {
            ...
            expect(node.plan?.limit).toBeUndefined();
            expect(node.plan?.offset).toBeUndefined();
            expect(node.plan?.orderBy).toBeUndefined();
        });
    });
});
