...
const apis_1 = require("../api/routes/apis");
...
const mockCreateSearchService = search_1.createSearchService;
// Get the first call (module initialization) which is what the module actually uses
const mockSearchService = mockCreateSearchService.mock.results[0].value;
// Mock the registry
jest.mock('../registry', () => ({
    apiRegistry: {
...
const mockApiRegistry = registry_1.apiRegistry;
describe('API Routes', () => {
    let mockRes;
    let mockReq;
    beforeEach(() => {
    ...
    describe('PATCH /apis/:id/status', () => {
        it('indexes API in ChromaDB when status changes to ACTIVE', async () => {
            ...
            const routeHandler = apis_1.apiRouter.routes.get('PATCH:/apis/:id/status')?.handler;
