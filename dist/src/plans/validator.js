...
function validateJoins(plan, issues, schema) {
    ...
    plan.join.forEach((joinEntry, i) => {
        ...
        const hasDerivedJoin = schema.relationships?.some((rel) => (rel.fromTable === plan.entity && rel.toTable === joinTable) ||
...
// Helper functions
// ------------------------------------------------------------------
function getAllAvailableColumns(plan, schema) {
    const available = {};
    ...
    const joins = plan.join ?? plan.joins ?? [];
