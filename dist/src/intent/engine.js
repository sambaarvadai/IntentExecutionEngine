...
// ------------------------------------------------------------------
class IntentEngine {
    constructor(anthropic, searchService // optional
    ) {
    ...
    async execute(request) {
        ...
        try {
            ...
            if (this.searchService) {
                const cacheResult = await this.searchService.checkCache(request.prompt);
                if (cacheResult.hit && cacheResult.match) {
                    const api = cacheResult.match.api;
                    const graph = api.executionGraph;
    ...
    // Private Helpers
    // ------------------------------------------------------------------
    validateGraphConstraints(graph, options) {
        const maxNodes = options?.maxNodes ?? 10; // default 10
        if (graph.nodes.length > maxNodes) {
            throw new graphParser_1.IntentParseError(`Generated graph has ${graph.nodes.length} nodes which exceeds ` +
