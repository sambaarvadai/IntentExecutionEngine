"use strict";
// ------------------------------------------------------------------
// API Management Routes
// ------------------------------------------------------------------
Object.defineProperty(exports, "__esModule", { value: true });
exports.apiRouter = void 0;
exports.createRouter = createRouter;
const registry_1 = require("../registry");
const search_1 = require("../../search");
// Initialize search service
const searchService = (0, search_1.createSearchService)(process.env.VOYAGE_API_KEY || '', process.env.CHROMA_URL || 'http://localhost:8000');
// Initialize search service
searchService.init().catch(err => {
    console.error('Failed to initialize search service:', err);
});
// ------------------------------------------------------------------
// Simple Router Implementation
// ------------------------------------------------------------------
function createRouter() {
    const routes = new Map();
    return {
        get(path, handler) {
            routes.set(`GET:${path}`, { method: 'GET', handler });
        },
        patch(path, handler) {
            routes.set(`PATCH:${path}`, { method: 'PATCH', handler });
        },
        post(path, handler) {
            routes.set(`POST:${path}`, { method: 'POST', handler });
        },
        routes
    };
}
// ------------------------------------------------------------------
// API Router
// ------------------------------------------------------------------
exports.apiRouter = createRouter();
// GET /apis
exports.apiRouter.get('/apis', async (req, res) => {
    try {
        const { status, limit, offset, route, method, label } = req.query || {};
        const result = await registry_1.apiRegistry.query({
            status: status,
            limit: limit ? parseInt(limit) : undefined,
            offset: offset ? parseInt(offset) : undefined,
            route,
            method,
            label
        });
        res.json(result);
    }
    catch (error) {
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
// GET /apis/:id
exports.apiRouter.get('/apis/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const api = await registry_1.apiRegistry.get(id);
        res.json(api);
    }
    catch (error) {
        res.status(404).json({ error: 'API not found' });
    }
});
// PATCH /apis/:id/status
exports.apiRouter.patch('/apis/:id/status', async (req, res) => {
    try {
        const { id } = req.params;
        const { status } = req.body;
        // Validate status
        const validStatuses = ['GENERATED', 'DRAFT', 'REVIEW', 'ACTIVE', 'DEPRECATED'];
        if (!validStatuses.includes(status)) {
            return res.status(400).json({
                error: 'Invalid status. Must be one of: GENERATED, DRAFT, REVIEW, ACTIVE, DEPRECATED'
            });
        }
        // Update API status
        const updatedApi = await registry_1.apiRegistry.updateStatus(id, status);
        // Wire search service integration
        if (updatedApi.status === 'ACTIVE' || updatedApi.status === 'REVIEW') {
            // Fire and forget — don't block the response on indexing
            searchService.indexAPI(updatedApi).catch(err => {
                console.error('Failed to index API in ChromaDB:', err);
            });
        }
        if (updatedApi.status === 'DEPRECATED') {
            searchService.removeFromIndex(updatedApi.id).catch(err => {
                console.error('Failed to remove API from ChromaDB:', err);
            });
        }
        res.json(updatedApi);
    }
    catch (error) {
        if (error instanceof Error && error.message.includes('not found')) {
            return res.status(404).json({ error: 'API not found' });
        }
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
// Export router for registration
exports.default = exports.apiRouter;
//# sourceMappingURL=apis.js.map