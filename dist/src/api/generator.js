...
class APIGenerator {
    constructor(intentEngine, config = {}) {
    ...
    async generateAPI(request) {
        ...
        try {
            ...
            const api = await this.generateAPIDefinition(primaryPlan, graph, request, constraints);
    ...
    // ------------------------------------------------------------------
    async generateExecutionGraph(intent) {
    ...
    async generateAPIDefinition(plan, // ← now nullable
    graph, // ← new param
    request, constraints) {
    ...
    generateRoute(plan, intent, graph) {
        if (!plan?.entity) {
    ...
    generateLabel(plan, intent, graph) {
        if (!plan?.entity) {
    ...
    generateDescription(plan, intent, graph) {
