...
export interface ValidationIssue {
  ...
  field: string;       // which part of the plan the issue is on e.g. "where[0].op"
  message: string;     // human-readable description
  suggestion?: string; // LLM-friendly correction hint
}

export interface ValidationResult {
  valid: boolean;
  issues: ValidationIssue[];
  // Structured summary fed back to LLM for self-correction
  llmFeedback?: string;
}
...
function validateEntity(
  plan: QueryPlan,
  issues: ValidationIssue[],
  schema: ReturnType<typeof getSchemaMetadata>
): void {
  if (!plan.entity) {
...
function validateSelect(
  ...
  if (!plan.select || plan.select.length === 0) return;
...
function validateWhereConditions(
  conditions: any[],
  issues: ValidationIssue[],
  schema: ReturnType<typeof getSchemaMetadata>,
  plan: QueryPlan,
...
function validateJoins(
  ...
  if (!plan.join || plan.join.length === 0) return;
...
function buildLlmFeedback(issues: ValidationIssue[]): string {
