...
export class PlanHydrator {
  // ------------------------------------------------------------------
  // Main hydration logic
  // ------------------------------------------------------------------

  async hydrate(context: HydrationContext): Promise<HydrationResult> {
    ...
    try {
      ...
      throw new ValidationError(`Plan hydration failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private injectParameters(
    ...
    if (plan.select && plan.select.length > 0) {
      plan.select = plan.select.map((field: string) => 
        this.injectValue(field, params, paramDefMap, warnings, 'select')
      );
    }
    ...
    if (plan.orderBy) {
      ...
      plan.orderBy = orderEntries.map((entry: any) => ({
    ...
    if (plan.groupBy && plan.groupBy.length > 0) {
      plan.groupBy = plan.groupBy.map((field: string) => 
        this.injectValue(field, params, paramDefMap, warnings, 'groupBy')
      );
    }
  }
    ...
    for (const paramName of paramRefs) {
      ...
      if (params[paramName] === undefined) {
        if (paramDef.required) {
          throw new ValidationError(`Required parameter '${paramName}' is missing for ${context}`);
        } else {
  ...
  private extractParameterReferences(value: string): string[] {
  ...
  private validateParameterValue(value: any, paramDef: any, context: string, paramName: string): any {
    // Type validation
    switch (paramDef.type) {
      ...
      case 'number':
        ...
        if (isNaN(num)) {
          throw new ValidationError(`Parameter '${paramName}' in ${context} must be a number, got ${typeof value}`);
        }
        return num;
      case 'boolean':
        ...
        throw new ValidationError(`Parameter '${paramName}' in ${context} must be a boolean, got ${typeof value}`);
      case 'array':
        if (!Array.isArray(value)) {
          throw new ValidationError(`Parameter '${paramName}' in ${context} must be an array, got ${typeof value}`);
        }
        return value;
      default:
        throw new ValidationError(`Unknown parameter type '${paramDef.type}' for parameter '${paramName}' in ${context}`);
    }
  }
  ...
  async hydrateWithDefaults(context: HydrationContext): Promise<HydrationResult> {
    ...
    const contextWithDefaults: HydrationContext = {
      ...context,
  ...
  async validateParameters(context: HydrationContext): Promise<string[]> {
    const warnings: string[] = [];
  ...
  private applyValidationRule(value: any, rule: any, paramName: string): void {
    switch (rule.type) {
      case 'min':
  ...
  private static instance: PlanHydrator;
...
export const planHydrator = PlanHydrator.getInstance();
