...
import { graphRouter } from '../api/routes/graphs';
...
const MockedIntentEngine = IntentEngine as jest.MockedClass<typeof IntentEngine>;
...
function createTestApp() {
  ...
  for (const [key, { handler }] of graphRouter.routes) {
...
describe('POST /graphs/validate', () => {
  let testApp: any;
  let mockEngine: jest.Mocked<IntentEngine>;
  ...
  it('should return 422 for IntentParseError', async () => {
    const parseError = new IntentParseError(
