...
describe('Transform Node Factories', () => {
  ...
  describe('mergeByKey', () => {
    it('joins customers with orders correctly', () => {
      ...
      const priya = result.rows.find((row: any) => row.id === 2)
      ...
      const anu = result.rows.find((row: any) => row.id === 4)
  ...
  describe('sortRows', () => {
    ...
    it('handles null values correctly', () => {
      ...
      const sortedNames = result.rows.map((row: any) => row.name)
