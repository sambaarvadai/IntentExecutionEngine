...
describe('VoyageEmbeddings', () => {
  ...
  beforeEach(() => {
    embeddings = new VoyageEmbeddings(mockApiKey);
    mockFetch.mockClear();
  });
  ...
  describe('embed', () => {
    ...
    it('throws on non-ok response', async () => {
      ...
      await expect(embeddings.embed(text)).rejects.toThrow(
  ...
  describe('embedDocument', () => {
    ...
    it('throws on non-ok response', async () => {
      ...
      await expect(embeddings.embedDocument(text)).rejects.toThrow(
        'Voyage API error: 400 Bad Request'
      );
    });
